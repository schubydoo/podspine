...
By default (`full`) Podspine pre-splits every chapter to disk at ingest, so each
book uses roughly **twice** its source size (the original plus the per-chapter
files) and serves instantly.
...
work exactly as in `full` mode.

- **`PODSPINE_CACHE_SIZE`** bounds the on-demand cache (least-recently-played
  chapters are evicted first). `0`/`off` keeps everything ever played.
...
- Ingest takes the **same** time in either mode — `saver` saves disk, not
  ingest time. Folder-of-MP3s books are always `full` (their tracks are whole
  files, not splits).

`saver` suits large libraries on small disks; keep `full` if disk is cheap and
you want every play to start instantly.
