...
fn run_ingest(task: &IngestTask, split_workers: usize) -> Result<PreparedBook, ScanError> {
    match &task.kind {
        ...
        IngestKind::TrackFolder { files, transcode } => {
            ingest_track_folder(task, files, *transcode)
        }
    }
}
...
fn ingest_single(
    ...
    // `split` covers all extract paths: full/saver chapter splits, whole-file
    // transcode, and the opt-in faststart remux (all funnel through ffmpeg).
    ...
    let episodes = if chapterless && transcoding {
        ...
        // Split every chapter to record its real byte size, then discard the
        // files. The http layer regenerates each on demand (the stream copy is
        // deterministic, so the regenerated bytes match the recorded length).
        // Reuse the same parallel splitter as `full` mode, so a many-chapter
        // saver book splits across cores instead of one chapter at a time; the
        // process-wide ffmpeg gate still bounds concurrency.
        //
        // Split into an ISOLATED temp dir, not the live `books/<id>` dir. The
        // parallel splitter publishes every chapter at once, so writing into the
        // live dir would expose freshly regenerated bytes at the serve path
        // (`resolve_audio_target` recomputes `books/<id>/<n>`) while the feed
        // still advertises the old `enclosure length`, and cache eviction could
        // race the cleanup. The temp dir is under `data_dir` (skipped by the
        // walk) and is not an indexed book id (skipped by eviction). Record the
    ...
    // art. `cover` includes the browse-UI thumbnail regeneration below it.
...
struct FolderTrack {
    ...
    title: String,
    /// Audio codec of the source (from ffprobe), which picks the output
    /// container when the track is re-encoded.
    codec: Option<String>,
...
fn plan_track_folder(
    ...
    let stored_transcode_ok = |existing: &BookRow| {
        // Transcode toggle guard, as in `plan_book`: a `PODSPINE_TRANSCODE`
        // flip changes a track's container and its recorded `byte_length`,
        // and touches no source mtime.
        ...
        && stored_transcode_ok(&existing)
    {
        ...
        //
        // A transcoded track is the exception: it lives under `<data_dir>`
        ...
        // that is NOT transcoded is a pre-6.2 verbatim copy, and the empty
...
/// read-only library. A folder can hold both.
fn ingest_track_folder(
    ...
    for (idx, t) in tracks.iter().enumerate() {
        // A re-encoded track lands under `<data_dir>` and is served from
        // there. It carries no `source_path`, which is how the serve layer
        // knows not to look in the library, and how nothing tries to
        // regenerate it: a re-encode is not byte-reproducible.
        let (file_path, source_path, byte_length) = if t.encoding == Encoding::Copy {
            ...
            let out_ext = episode_ext(t.codec.as_deref(), t.encoding);
            let ep = transcode_whole(
                &t.path,
                &task.book_out,
                idx,
                out_ext,
            ...
            if let Some(name) = ep.path.file_name().and_then(|n| n.to_str()) {
    ...
    Ok(PreparedBook {
        book,
        episodes,
        // A track that streams in place from the library leaves nothing under
        // `<data_dir>`. Any verbatim copy a pre-6.2 ingest wrote there is dead
        // weight, but ONLY once the new rows point into the library. A pre-6.2
        // book's live rows still name those copies, so deleting them before
        // the commit would 404 that book until the commit landed (Greptile
        // P1). A folder with re-encoded tracks keeps exactly the files this
        // ingest wrote, and drops the rest, which is what retires the output
        // of a previous transcode setting.
        sweep: if written.is_empty() {
...
fn scan_library_with_progress(
    ...
    tracing::debug!(
        target: "podspine::scan_timing",
        stage = "scan_total",
        ms = scan_start.elapsed().as_secs_f64() * 1e3,
        "stage timing"
    );
    summary
}
...
/// Log one scan STAGE's wall-clock at debug, under the `podspine::scan_timing`
/// target so a profiling run can filter and aggregate it (see
/// `scripts/bench-scan.sh`). Bracket a stage with
/// `let t = Instant::now(); …; log_stage(id, "probe", t);`. Inert at the default
/// `info` level, so nothing is emitted in normal operation. `stage` is a short
/// fixed label (`probe`, `resolve`, `split`, `cover`, `index`, `book_total`,
/// `scan_total`).
fn log_stage(book: &str, stage: &str, start: std::time::Instant) {
    tracing::debug!(
        target: "podspine::scan_timing",
        book,
        stage,
...
mod tests {
    ...
    #[test]
    fn log_stage_is_safe_without_a_subscriber() {
        // Per-stage timing must never affect a real scan: with no tracing
        // subscriber installed (debug disabled), it is a no-op. Prove it does not
        // panic. Full per-stage output is exercised by the ffmpeg-gated scans.
        log_stage("book-x", "probe", std::time::Instant::now());
    }
