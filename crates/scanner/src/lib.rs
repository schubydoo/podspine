...
pub fn scan_book_as(
    ...
    let episodes = if chapterless && transcoding {
        ...
        // Split every chapter to record its real byte size, then delete the
        ...
        // process-wide ffmpeg gate still bounds concurrency. Peak extra disk is
        // the full split set briefly, then back to zero once deleted.
        let eps = split_book_encoded(input, &book_out, &cuts, out_ext, enc)?;
        for ep in &eps {
            std::fs::remove_file(&ep.path).map_err(|source| ScanError::Io {
                path: ep.path.clone(),
                source,
